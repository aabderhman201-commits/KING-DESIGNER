...
export function ProfilePage({ userId, onNavigate }: ProfilePageProps) {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    const { data: servicesData } = await supabase
      ...
      .eq('user_id', targetId)
      .order('created_at', { ascending: false });
    setServices(servicesData as DesignerService[]);
    ...
    const { data: portfolioData } = await supabase
      ...
      .eq('user_id', targetId)
      .order('created_at', { ascending: false });
    setPortfolioSections(portfolioData as PortfolioSection[]);
    ...
    const { data: foldersData } = await supabase
      ...
      .eq('user_id', targetId)
      .order('created_at', { ascending: false });
    setPortfolioFolders(foldersData as PortfolioFolder[]);
  ...
  const handleGifUpload = async () => {
    if (!currentUser || !gifFile) return;
    if ((currentUser.vip_level || 0) < 3) return;
    const path = `${currentUser.id}/gif-avatar-${Date.now()}-${gifFile.name}`;
    const { error } = await supabase.storage.from(STORAGE_BUCKETS.AVATARS).upload(path, gifFile);
    if (error) return;
    const { data: { publicUrl } } = supabase.storage.from(STORAGE_BUCKETS.AVATARS).getPublicUrl(path);
    await supabase.from('profiles').update({ avatar_gif_url: publicUrl }).eq('id', currentUser.id);
    setGifPreview(null);
    setGifFile(null);
    await refreshProfile();
    await fetchProfile();
        ...
        <div className="px-4 md:px-6 pb-4 -mt-12 md:-mt-14">
          <div className="flex items-end justify-between mb-3">
            <div className="relative">
              ...
              {isOwn && gifPreview && (
                <div className="absolute -bottom-24 left-0 z-20 flex gap-1">
                  <button onClick={handleGifUpload} className="bg-gradient-to-r from-king-500 to-king-700 text-white rounded-lg px-2 py-1 text-xs">{t('saveChanges')}</button>
          ...
          {/* ID Copy */}
          <button onClick={handleCopyId} className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-king-500 transition-colors mb-2">
          ...
          <div className="flex items-center gap-4 flex-wrap text-xs text-gray-500 dark:text-gray-400">
            ...
            {user.whatsapp_number && <a href={`https://wa.me/${user.whatsapp_number.replace(/\D/g, '')}`} target="_blank" rel="noreferrer" className="text-success-600 hover:text-success-700">WhatsApp</a>}
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right flex flex-col items-end gap-2">
                      ...
                      {isOwn && (
                        <button
                          onClick={async () => { await supabase.from('designer_services').delete().eq('id', service.id); fetchProfile(); }}
                          className="text-error-500 hover:text-error-600"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
  ...
  const handleAddMedia = async (files: FileList) => {
    ...
    for (const file of Array.from(files)) {
      ...
      if (!error) {
        ...
        newMedia.push({ type: getFileType(file.name), url: publicUrl, name: file.name, size: file.size });
      }
    }
    const updatedMedia = [...(folder.media || []), ...newMedia];
    await supabase.from('portfolio_folders').update({ media: updatedMedia }).eq('id', folder.id);
    setUploading(false);
    onUpdate();
  };

  const handleRemoveMedia = async (index: number) => {
    const updatedMedia = (folder.media || []).filter((_, i) => i !== index);
    await supabase.from('portfolio_folders').update({ media: updatedMedia }).eq('id', folder.id);
    onUpdate();
  };

  const handleRenameMedia = async (index: number) => {
    const item = folder.media?.[index];
    if (!item) return;
    const newName = prompt(t('editFolder'), item.name || '');
    if (newName === null) return;
    const updatedMedia = (folder.media || []).map((m, i) => i === index ? { ...m, name: newName.trim() } : m);
    await supabase.from('portfolio_folders').update({ media: updatedMedia }).eq('id', folder.id);
    onUpdate();
  };
  ...
  return (
    <div className="space-y-3" onContextMenu={handleContextMenu}>
      ...
      {(!folder.media || folder.media.length === 0) ? (
        ...
        <div
          className="grid grid-cols-2 md:grid-cols-3 gap-3 select-none"
...
function AddServiceModal({ userId, onClose }: { userId: string; onClose: () => void }) {
  ...
  const handleSave = async () => {
    if (!name.trim()) return;
    setSaving(true);
    await supabase.from('designer_services').insert({
      user_id: userId,
