...
import { Home, Users, MessageSquare, Bell, User, Moon, Sun, Search, Shield, Briefcase, Globe } from 'lucide-react';
...
export type Page = 'home' | 'friends' | 'messages' | 'notifications' | 'profile' | 'admin' | 'search' | 'service-requests' | 'verification' | 'post-detail';
...
export function Layout({ currentPage, onNavigate, children, unreadNotifications, unreadMessages }: LayoutProps) {
    ...
    <div className="min-h-screen bg-surface-light dark:bg-surface-dark flex">
      ...
      <aside className={`hidden md:flex flex-col w-64 fixed top-0 bottom-0 border-${isRTL ? 'l' : 'r'} border-king-100 dark:border-surface-dark-border bg-white dark:bg-surface-dark-alt p-4 z-30 ${isRTL ? 'right-0' : 'left-0'}`}>
        <div className="flex items-center justify-center mb-8 px-2">
          <img src="/تصميم_بدون_عنوان.png" alt="King Design" className="w-full max-w-[190px] h-16 object-contain" />
        </div>
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 glass border-t border-king-100 dark:border-surface-dark-border">
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 z-30 glass border-b border-king-100 dark:border-surface-dark-border px-4 py-3 flex items-center justify-between">
        <div className="flex items-center" onClick={() => onNavigate('home')}>
          <img src="/تصميم_بدون_عنوان.png" alt="King Design" className="w-32 h-10 object-contain" />
        </div>
