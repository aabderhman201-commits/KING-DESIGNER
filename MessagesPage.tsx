...
import { Send, Paperclip, Mic, X, Edit2, Trash2, Check, Play, Pause, Search, MessageSquare, Reply, ChevronLeft, CheckCheck, Eye, AlertCircle, Sparkles } from 'lucide-react';
...
export function MessagesPage({ targetUserId }: MessagesPageProps) {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    if (messagesResult.error || requestsResult.error) {
    ...
    setMessageRequests((requestsResult.data || []) as MessageRequest[]);
    ...
    <div className="flex h-[calc(100vh-8rem)] md:h-[calc(100vh-3rem)] -m-4 md:-m-6">
      ...
      <div className={`w-full md:w-80 border-r border-king-100 dark:border-surface-dark-border flex flex-col ${activeConversation ? 'hidden md:flex' : 'flex'}`}>
        ...
        {profile && !profile.is_admin && (
          <button
            onClick={() => startConversationWith('00000000-0000-0000-0000-000000000010')}
            className="mx-4 mt-2 w-[calc(100%-2rem)] flex items-center justify-center gap-2 rounded-xl border border-king-200 dark:border-surface-dark-border bg-white dark:bg-surface-dark-card px-3 py-2 text-sm text-king-700 dark:text-king-300 hover:bg-king-50 dark:hover:bg-surface-dark-alt transition-colors"
          >
            <Sparkles className="w-4 h-4" /> {t('adminSuggestions')}
          </button>
        )}
      ...
      <div className={`flex-1 flex flex-col ${activeConversation ? 'flex' : 'hidden md:flex'}`}>
        {!activeConversation ? (
          ...
          <>
            ...
            {isMessageRequest && (
              <div className="px-4 py-2 bg-warning-50 dark:bg-warning-700/20 border-b border-warning-100 dark:border-warning-700/30">
